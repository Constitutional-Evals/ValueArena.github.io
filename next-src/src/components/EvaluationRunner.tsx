...
export function EvaluationRunner() {
  ...
  const [limits,setLimits] = useState({max_models:8,max_scenarios:200,max_runtime_seconds:14400,max_disk_gb:500,allow_own_keys:true,allow_public_results:true});
  ...
  function addModel() {
    ...
    if (selected.length >= limits.max_models) { setError(`The panel can contain up to ${limits.max_models} models.`); return; }
  ...
  async function upload(file?: File) {
    ...
    try {
      ...
      const text = await file.text(); const rows = parseScenarios(text,limits.max_scenarios);
  ...
  function buildRequest(includeKeys = true) {
    return { name, engine, max_runtime_seconds:runtime, advanced_spec: parseAdvancedSpec(advanced), models: selected, custom_models: custom.filter(m => selected.includes(m.id)),
        criteria: criteria.split('\n').map(s => s.trim()).filter(Boolean), constitution_name: constitution === 'custom' ? 'Custom' : label(constitution),
        scenario_source: source, scenario_count: count, scenarios: source === 'custom' ? parseScenarios(scenarioText,limits.max_scenarios) : [], visibility,
  ...
  if (selected.length > limits.max_models) blockers.push(`Remove models to stay within your ${limits.max_models}-model limit.`);
  ...
  else if (credits === null) blockers.push('Waiting for your credit balance.');
  else if (credits < runtime) blockers.push(`This run reserves ${Math.ceil(runtime/60)} minutes; you have ${Math.floor(credits/60)}. Reduce the execution allowance, request credits, or use your provider keys.`);
  if (!Number.isInteger(runtime) || runtime < 300 || runtime > limits.max_runtime_seconds) blockers.push(`Set an execution allowance between 5 and ${Math.floor(limits.max_runtime_seconds/60)} minutes.`);
  ...
  if (source === 'custom') {
    try { parseScenarios(scenarioText,limits.max_scenarios); } catch(e) { blockers.push((e as Error).message); }
  } else if (!Number.isInteger(count) || count < 1 || count > limits.max_scenarios) blockers.push(`Choose between 1 and ${limits.max_scenarios} scenarios.`);
    ...
    {tab === 'new' && access==='approved' && <form className="evaluation-form eval-workspace" noValidate onSubmit={submit}>
      <div className="eval-main">
        <section className="eval-section"><header><span>01</span><h2>Model panel</h2></header><p>Each model answers the scenarios and judges the responses. Choose 2–{limits.max_models} models.</p>
          <div className="eval-model-presets">{models.map(model => <label className="evaluation-model" key={model.id}><input type="checkbox" checked={selected.includes(model.id)} disabled={!selected.includes(model.id) && selected.length >= limits.max_models} onChange={e => setSelected(ids => e.target.checked ? [...ids, model.id] : ids.filter(id => id !== model.id))} />{model.label}</label>)}</div>
      ...
      <aside className="eval-setup"><h2>Run setup</h2><label>Evaluation name<input required maxLength={120} value={name} onChange={e => setName(e.target.value)} placeholder="Humor / Qwen comparison" /></label><label>Engine<select value={engine} onChange={e => setEngine(e.target.value)}><option value="native">Native EigenBench</option><option value="inspect">Inspect</option></select></label>
        ...
        <label>Temporary storage (GB)<input disabled={!ownKeys && !isAdmin} type="number" min={50} max={limits.max_disk_gb} step={10} value={ownKeys || isAdmin ? disk : 100} onChange={e => setDisk(Number(e.target.value))} /><button type="button" disabled={!ownKeys && !isAdmin} onClick={()=>setDisk(limits.max_disk_gb)}>Max allowed</button><small>Model cache and working files. Released after results are saved.</small></label>
        ...
        <label>Execution allowance (minutes)<input type="number" min={5} max={Math.floor(limits.max_runtime_seconds/60)} value={runtime/60} onChange={e=>setRuntime(Number(e.target.value)*60)}/><button type="button" onClick={()=>setRuntime(limits.max_runtime_seconds)}>Max allowed</button></label><AdvancedConfiguration value={advanced} onChange={setAdvanced} modelIds={selected} configVersion={JSON.stringify([advanced, name, engine, selected, custom, criteria, source, count, scenarioText])} getRequest={() => buildRequest(false)} />
        <div className="eval-submit"><span>{selected.length} models · {typeof advancedCount === 'number' ? advancedCount : source === 'airiskdilemmas' ? count : 'custom'} scenarios</span><button className="button-primary" disabled={busy} aria-describedby={blockers.length ? "evaluation-blockers" : undefined}>{busy ? 'Preparing…' : 'Run evaluation →'}</button>{blockers.length>0 && <div id="evaluation-blockers" tabIndex={-1} aria-live="polite"><strong>Before you can run</strong><ul>{blockers.map((message,i)=><li key={i}>{message}</li>)}</ul></div>}{error && <p role="alert">{error}</p>}<small>Runs continue in the background. Automatic stop after {Math.round(runtime/60)} minutes; unused service time is returned.</small></div>
      </aside>
    </form>}
