  ...
  return <>
    ...
    {tab === 'new' && access==='approved' && <form className="evaluation-form eval-workspace" onSubmit={submit}>
      ...
      <aside className="eval-setup"><h2>Run setup</h2><label>Evaluation name<input required maxLength={120} value={name} onChange={e => setName(e.target.value)} placeholder="Humor / Qwen comparison" /></label><label>Engine<select value={engine} onChange={e => setEngine(e.target.value)}><option value="native">Native EigenBench</option><option value="inspect">Inspect</option></select></label>
        ...
        <label>Temporary storage (GB)<input disabled={!ownKeys && !isAdmin} type="number" min={50} max={limits.max_disk_gb} step={10} value={ownKeys || isAdmin ? disk : 100} onChange={e => setDisk(Number(e.target.value))} /><small>Model cache and working files. Released after results are saved.</small></label>
        ...
        <label>Execution allowance (minutes)<input type="number" min={5} max={Math.floor(limits.max_runtime_seconds/60)} value={runtime/60} onChange={e=>setRuntime(Number(e.target.value)*60)}/></label><AdvancedConfiguration value={advanced} onChange={setAdvanced} modelIds={selected} configVersion={JSON.stringify([advanced, name, engine, selected, custom, criteria, source, count, scenarioText])} getRequest={() => buildRequest(false)} />
        <div className="eval-submit"><span>{selected.length} models · {typeof advancedCount === 'number' ? advancedCount : source === 'airiskdilemmas' ? count : 'custom'} scenarios</span><button className="button-primary" disabled={busy || access!=='approved' || !submissionsEnabled || !advancedValid || selected.length < 2 || selected.length > limits.max_models || (!ownKeys && (!enabled || (credits ?? 0) < runtime))}>{busy ? 'Preparing…' : 'Run evaluation →'}</button>{!advancedValid && <small role="alert">Fix the JSON in Advanced configuration before submitting.</small>}<small>Runs continue in the background. Automatic stop after {Math.round(runtime/60)} minutes; unused service time is returned.</small></div>
      </aside>
    </form>}
