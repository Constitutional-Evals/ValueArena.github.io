...
The homepage highlights recent experiments and model rankings. The main sections are:

- **Compare** (`/compare/`) — Pick two models and a constitution (e.g. Kindness, Humor, Sarcasm), then chat side-by-side. Vote on which model better reflects the chosen value. Uses OpenRouter for inference directly from the browser.
...
Open `http://localhost:3000`. To preview the production export:
...
Open `http://localhost:8000`. Source changes belong in `next-src/`; the published
root files are generated by the build workflow.
...
`/explore/` shows model rankings with published confidence intervals and paired
constitution scatterplots. Search models, filter families, select a point to
read its judgments, or share the current view by URL. Comparisons require the
same experiment, model panel, and recorded dataset, scoring, generation, and
sampling settings. Each axis uses its own run's Elo scale; intervals are
marginal, not a joint confidence region.
...
values. Figures are resized WebP copies; numbers and plotted uncertainties are
preserved. Update these together with `ResearchStory.tsx` when the paper changes.
The overview deliberately identifies these as paper results rather than a live
aggregate. LAISR Lab attribution appears in the introduction and footer.
