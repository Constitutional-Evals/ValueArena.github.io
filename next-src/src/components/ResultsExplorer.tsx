...
export function ResultsExplorer({ embedded = false }: { embedded?: boolean }) {
  ...
  return <section className={`results-explorer${embedded ? ' home-tradeoff' : ''}`} aria-label="Interactive results">
    ...
    {/deprecated/i.test(`${xRun?.name} ${xRun?.note} ${yRun?.name} ${yRun?.note}`) && <p className="chart-message" role="status">This selection includes a deprecated run. See its notes before interpreting the results.</p>}
    {(x.error || (view === 'tradeoff' && y.error)) ? <p className="chart-message" role="alert">{x.error || y.error}</p> : view === 'tradeoff' && !ySlug ? <p className="chart-message">This experiment has no second constitution. Choose a run from a multi-constitution experiment.</p> : !x.value || (view === 'tradeoff' && !y.value) ? <p className="chart-message" role="status">Loading scores and run settings…</p> : issues.length ? <div className="chart-message" role="status"><strong>These runs cannot be plotted as a matched comparison.</strong><ul>{issues.map(issue => <li key={issue}>{issue}</li>)}</ul><a href={runURL(xSlug)}>Inspect the first run →</a></div> : !rows.length ? <p className="chart-message">No matching models. Try clearing the search or family filter.</p> : <>
      {!embedded && <header className="chart-heading"><h2>{view === 'ranking' ? `${label(xRun)}: model rankings` : `${label(xRun)} and ${label(yRun)}`}</h2><p>{view === 'ranking' ? 'Higher scores indicate stronger expression of the selected constitution.' : 'Each axis shows Elo within its own run. Higher is more of that trait—not necessarily better.'}</p></header>}
      {view === 'ranking' ? <div className="ranking-figure" role="group" aria-label={`${label(xRun)} rankings with confidence intervals`}>
        ...
        {points.map(p => { const cx=position(p.x.elo_mean,domain,70,680), cy=400-position(p.y.elo_mean,yDomain,0,370), ix=interval(p.x), iy=interval(p.y), active=focused===p.name, color=colorOf(p.name); return <g key={p.name} opacity={focused && !active ? .55 : 1}>
          ...
          <circle cx={cx} cy={cy} r={active?7:5} fill={color} stroke="var(--bg)" strokeWidth="1.5" pointerEvents="none" />
      ...
      <p className="chart-footnote">{embedded ? `${paired.length} shared models. Higher scores mean more of each trait. Bars show separate 95% intervals.` : view==='ranking' ? 'Points show mean Elo; lines show published 95% intervals where available.' : `${paired.length} shared models. Lines show separate 95% intervals, not a joint confidence region. Recorded settings match; scenario contents and model revisions are not independently verified.`}</p>
    </>}
    {chartReady && <aside className="chart-inspector" aria-label="Selected model details">
      ...
      {detail ? <div className="chart-detail"><strong><ModelLogo name={x.value?.meta.models?.[detail.model_name]?.id || detail.model_name} size={22} />{detail.model_name}</strong><Score label={label(xRun)} row={detail} /><a href={transcriptURL(xSlug,detail.model_name)}>Read {label(xRun).toLowerCase()} judgments →</a>{view==='tradeoff' && yDetail && <><Score label={label(yRun)} row={yDetail} /><a href={transcriptURL(ySlug,detail.model_name)}>Read {label(yRun).toLowerCase()} judgments →</a></>}</div> : <p>Hover, tap, or focus a point to see scores. Select it to keep the details open.</p>}
    </aside>}
